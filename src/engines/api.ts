...
async function getWebPlayerDevToken(): Promise<string> {
  ...
  throw new Error(
    ...
    "This may mean Apple changed their web player structure.\n" +
    "Use --engine native to fall back to Music.app control."
  );
}
...
async function apiRequest<T>(
  ...
  if (!mediaUserToken) {
    throw new Error(
      "No media-user-token found. Run: aria auth import --browser safari\n" +
      "Or paste it manually: aria auth token <token>"
    );
  }
  ...
  if (!res.ok) {
    if (res.status === 401 || res.status === 403) {
      throw new Error(
        "Authentication failed. Your media-user-token may be expired.\n" +
        "Re-import: aria auth import --browser safari"
      );
    }
...
export class ApiEngine implements MusicEngine {
  ...
  async play(_query?: string): Promise<void> {
    throw new Error("Playback control requires the native engine. Use: aria --engine native play");
  }

  async pause(): Promise<void> {
    throw new Error("Playback control requires the native engine. Use: aria --engine native pause");
  }

  async resume(): Promise<void> {
    throw new Error("Playback control requires the native engine. Use: aria --engine native resume");
  }

  async next(): Promise<void> {
    throw new Error("Playback control requires the native engine. Use: aria --engine native next");
  }

  async previous(): Promise<void> {
    throw new Error("Playback control requires the native engine. Use: aria --engine native prev");
  }

  async seek(_seconds: number): Promise<void> {
    throw new Error("Playback control requires the native engine.");
  }

  async setVolume(_level: number): Promise<void> {
    throw new Error("Volume control requires the native engine.");
  }

  async getVolume(): Promise<number> {
    throw new Error("Volume requires the native engine.");
  }

  async getStatus(): Promise<PlaybackState> {
    throw new Error("Playback status requires the native engine.");
  }
  ...
  async search(query: string, types: SearchType[], limit = 20): Promise<SearchResults> {
    ...
    const data = await apiRequest<AMResponse>("/catalog/us/search", {
      term: query,
      types: amTypes,
      limit: String(limit),
    });
  ...
  async addToQueue(_trackId: string): Promise<void> {
    throw new Error("Queue management requires the native engine.");
  }
  ...
  async getPlaylistTracks(playlistId: string): Promise<Track[]> {
    const data = await apiRequest<AMResponse>(`/me/library/playlists/${playlistId}/tracks`, {
      limit: "100",
    });
  ...
  async getPlaylistInfo(playlistId: string): Promise<PlaylistDetails> {
    const data = await apiRequest<AMResponse>(`/me/library/playlists/${playlistId}`, {});
  ...
  async addToPlaylist(_playlistId: string, _trackIds: string[]): Promise<void> {
    throw new Error("Playlist editing via API not yet implemented. Use --engine native.");
  }
  ...
  async getDevices(): Promise<Device[]> {
    throw new Error("Device listing requires the native engine.");
  }
}
